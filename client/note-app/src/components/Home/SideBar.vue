...
<template>
  <Popup
    :popup="popup"
    @popup-event="handleSave"
    @popup-close="popup = false"
  />
  <div
    v-if="store._currentUser"
    class="w-60 h-full bg-slate-100 border-r border-r-slate-700 flex-shrink-0 p-4 flex flex-col"
  >
    <span
      @click="popup = true"
      class="text-black py-3 bg-blue-100 text-center font-bold cursor-pointer text-sm px-3 hover:bg-blue-300 duration-500"
      >Add New Category</span
    >
    <ul class="w-full mt-7">
      <li
        @click="emit('see-all')"
        class="text-lg mt-1 font-thin w-full cursor-pointer bg-orange-200 bg-opacity-0 hover:bg-opacity-100 py-2 px-3 rounded-sm flex gap-x-3"
      >
        <svg
