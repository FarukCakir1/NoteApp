<template>
  <div class="w-screen h-screen flex flex-col p-5">
    <h1 class="text-xl underline">Personal İnfo</h1>
    <div class="w-full h-2/6 flex items-center gap-x-8">
      <div class="border border-gray-600 p-4 rounded-sm bg-gray-100">
        <svg
      ...
      <h1 class="text-3xl">{{user.fullname}}</h1>
    </div>
    <hr class="bg-orange-500">
    <div class="w-full h-2/6 flex border-b border-b-orange-500 py-2 items-center gap-x-32">
        <div class="w-2/6 flex justify-start">
            <ul>
                <li class="text-xl mb-3">Your Categories</li>
                <li v-for="category in categories">{{category.name}}</li>
            </ul>
        </div>
        <div class="h-full bg-gray-500 w-[2px]"></div>
        <div class="w-2/6 flex justify-start">
            <ul>
                <li class="text-xl mb-5">Your Notes</li>
...
<script setup>
    ...
    const user = ref(store._currentUser)
